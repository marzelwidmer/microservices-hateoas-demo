...
//            's', 'S', 't', 'T', '3', 'V', 'W', 'X', 'Y', 'Z']
//    )
//    fun `Customer negative creation test - only gender M or F is allowed `(input: String) {
...
//    @ValueSource(strings = ["f", "F", "m", "M", "u", "U"])
//    fun `Customer creation test - it must have a valid gender fF 👧 - mM 🧔 - uU 🤷🏻`(input: String) {
//        Customer(id=UUID.randomUUID().toString(), firstName = FirstName(Faker().name.firstName()), lastName = LastName(Faker().name.lastName()), gender = Gender(input))
//    }
    ...
    fun `test customer firstName that is it not empty should throws IllegalStateException`() {
        assertThrows(IllegalArgumentException::class.java) {
            Customer(id=UUID.randomUUID().toString(), firstName = FirstName(""), lastName = LastName(Faker().name.lastName()))
        }
    }
    ...
    fun `test customer firstName with 1 characters should throws IllegalStateException`() {
        assertThrows(IllegalArgumentException::class.java) {
            val name = "A"
            Customer(id=UUID.randomUUID().toString(), firstName = FirstName(name), lastName = LastName(Faker().name.lastName()))
        }
    }
